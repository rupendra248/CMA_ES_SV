"""Redraw the protocol CDF for the UNCONDITIONED SVR. SUPERSEDED.

Reads out_errors.json, which holds the unconditioned per-sample errors. Figure 3
of the article is produced by region_errors.py followed by replot_region.py.
"""
import json
import numpy as np
import matplotlib
...
err = {k: np.sort(np.asarray(v)) for k, v in json.load(open("out_errors.json")).items()}
